...
function CountdownUnit({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex flex-col items-center">
      <div className="bg-foreground text-background w-14 h-14 md:w-16 md:h-16 rounded-xl flex items-center justify-center font-mono text-2xl md:text-3xl font-bold shadow-sm">
        {String(value).padStart(2, '0')}
      </div>
      <span className="text-[11px] uppercase tracking-widest text-muted-foreground mt-2 font-medium">{label}</span>
    </div>
  );
}

function CountdownTimer({ isoDate }: { isoDate: string | null }) {
  ...
  return (
    <div className="flex gap-2 mt-4">
      ...
      <div className="text-2xl font-bold text-muted-foreground/40 self-start mt-3">:</div>
...
export default function Dates() {
  return (
    <section className="py-20 bg-secondary/20 relative overflow-hidden" id="dates">
          ...
          <div className="grid sm:grid-cols-2 gap-4 md:gap-6">
            {dates.map((item, index) => (
              ...
              >
                ...
                <CountdownTimer isoDate={item.isoDate} />
              </motion.div>
            ))}
          </div>
        </motion.div>
